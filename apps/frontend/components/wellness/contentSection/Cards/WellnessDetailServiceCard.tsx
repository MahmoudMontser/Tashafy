...
type WellnessDetailServiceCardProps = {
  main: Pick<WellnessDetailMainContent, "servicesSection">;
  isRtl: boolean;
};
...
export default function WellnessDetailServiceCard({ main, isRtl }: WellnessDetailServiceCardProps) {
  const { title, subtitle, items } = main.servicesSection;
  ...
  return (
    <section
      ...
      dir={isRtl ? "rtl" : "ltr"}
      aria-labelledby="wellness-detail-services-heading"
    >
      <div className="flex w-full flex-col gap-0 text-start">
        <h2 id="wellness-detail-services-heading" className="text-[28px] font-semibold leading-8 tracking-[-0.56px] text-(--color-text-brand)">
          {title}
        </h2>
