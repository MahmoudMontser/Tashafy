...
import type { RehabilitationDetailSidebarContent } from "@/types/rehabilitation";
import { SaudiRiyal, Star } from "lucide-react";
import Link from "next/link";

export type RehabDetailSidebarProps = {
  sidebar: RehabilitationDetailSidebarContent;
  isRtl: boolean;
};

export function RehabDetailSidebar({ sidebar, isRtl }: RehabDetailSidebarProps) {
  const handleReserveClick = async (e: MouseEvent<HTMLAnchorElement>) => {
    e.preventDefault();
    await logAndOpenWhatsapp(
      {
        source: "rehab_detail_sidebar",
