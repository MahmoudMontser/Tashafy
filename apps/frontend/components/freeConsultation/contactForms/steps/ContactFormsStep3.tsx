...
const OPTIONS = [
  { value: "yes", labelKey: "optionYes" as const },
  { value: "no", labelKey: "optionNo" as const },
];

export type ContactFormsStep3Props = {
  locale: string;
  content: ContactFormStepYesNoContent;
  onNext: () => void;
  onPrevious: () => void;
  /** Distinct id when this layout is reused (e.g. step 4). */
  ariaHeadingId?: string;
};

export default function ContactFormsStep3({ locale, content, onNext, onPrevious, ariaHeadingId = "contact-form-step3-question" }: ContactFormsStep3Props) {
  ...
  return (
    ...
    >
      <div className="flex w-full flex-col gap-5">
        <h2 id={ariaHeadingId} className="w-full text-xl font-medium leading-8 text-(--color-text-primary)">
          {content.question}
        </h2>
